...
<div class="col-md-2">
    <div class="a_close"><a  href="edpTable"  data-toggle="tooltip" title="Close" class="pull-right" ><span class="glyphicon glyphicon-remove"></span></a></div><br>
                                        </div>
                                    </div>
      </div>
      <div class="modal-body" style="font-size:16px;">
        
    	<form class="form-horizontal" method="post" action="edpupdate_inventoryprocess">
   <div class="row">
  <input type="hidden" name="id" value="<?php echo $row['id'];?>" />
   <input type="hidden" name="idno" value="<?php echo $row['e_no'];?>" />
    <input type="hidden" name="mdep" value="<?php echo $row['monthly_dep'];?>" />
    <input type="hidden" name="amount" value="<?php echo $row['amount'];?>" />
       <input type="hidden" name="name" value="<?php echo $row['itemname'];?>" />
       <input type="hidden" name="sval" value="<?php echo $row['s_value'];?>" />
       <input type="hidden" name="elife" value="<?php echo $row['e_life'];?>" />
 ...
 $q = mysql_query("SELECT date ,eno FROM edp_ledgercard WHERE eno = '$id' order by date desc") or die (mysql_error());
 if($r = mysql_fetch_array($q)){?>
   <input type="text" class="form-control" name="pdate" value="<?php $d = $r['date'];
																	 ...
																	 echo $sd->format('M d, Y');?>" data-toggle="tooltip" title="<?php $date = $r['date'];
																															...
																															echo 'LAST UPDATED ON '.$isdate->format('M d, Y');?>" readonly/>                                                                                                               
    <?php }
	$dateacuired = mysql_query("SELECT date, eno FROM edp_ledgercard WHERE eno = '$id' GROUP by eno") or die (mysql_error());
