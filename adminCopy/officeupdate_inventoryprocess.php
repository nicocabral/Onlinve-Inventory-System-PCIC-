...
include('includes/dbconn.php');
	if(isset($_POST['save'])){
		...
		if($pd == $date_check){
		$check = mysql_query("SELECT date FROM office_ledgercard WHERE  oid = '$eno' Group by oid") or die (mysql_error());
	...
	$_SESSION['class'] = 'danger';exit();}else {
		...
		$sql = mysql_query("SELECT oid,cost,date,depreciation,accum_deplm,accum_deptm,bookvalue FROM office_ledgercard WHERE oid = '$eno'") or die (mysql_error());
		while($row=mysql_fetch_array($sql)){
			...
			$checkdate = mysql_query("SELECT date,oid FROM office_ledgercard WHERE Year(date) = '$dd' AND Month(date) = '$e' AND oid = '$eno'") or die (mysql_error());
	...
	$_SESSION['class'] = 'danger';exit();
		...
		if(number_format($sval,0,'.','')>number_format($bval,0,'.','')){
			$depreciated = "INSERT INTO office_depreciated VALUES(NULL,'$eno','$name','$datedep','$qty','$amount',0,'$deptm','$deptm','$bookvalue','Depreciated','$dateacquired')";
	...
	$_SESSION['class'] = 'danger';exit();}
				...
				$q = "INSERT INTO office_ledgercard VALUES(NULL,'$eno','$name','$date','$amount','$dep','$a','$b','$bval')";
