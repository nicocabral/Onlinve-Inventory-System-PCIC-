...
<?php unset($_SESSION['success_msg']);
      ...
      <a href="risTablecad" class="btn btn-default"  data-toggle="tooltip" title="CAD RIS" data-placement="bottom">CAD &emsp;<span class="badge" style="background-color:red;"><?php include('includes/dbconn.php');
	  											$q = mysql_query("SELECT COUNT(*) as newreq from pcic_requestitem WHERE status='Pending' AND department='CAD'") or die (mysql_error());
												if($row = mysql_fetch_array($q)){
													echo $row['newreq'];
													}
											
	  						?></span></a>&emsp;
        <a href="risTablemsd" class="btn btn-primary"  data-toggle="tooltip" title="MSD RIS" data-placement="bottom">MSD &emsp;<span class="badge" > <?php 
	  											$q = mysql_query("SELECT COUNT(*) as newreq from pcic_requestitem WHERE status='Pending' AND department='MSD'") or die (mysql_error());
												if($row = mysql_fetch_array($q)){
													echo $row['newreq'];
													}
 ...
 <?php include('includes/dbconn.php');
 	$sql = mysql_query("SELECT * FROM pcic_requestitem WHERE status !=' ' AND department='AFD' ORDER by timestamp desc") or die (mysql_error());
	$count = 0;
