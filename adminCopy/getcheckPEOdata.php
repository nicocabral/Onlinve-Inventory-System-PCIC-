...
<?php 
$dis = '';
if(@$_POST['filtercheck']==''){
	$dis = 'Disabled';}else {$dis = '';}?>
<form method="post" action="printcheckpeo">
	<input type="hidden" value="<?php echo @$_POST['filtercheck'];?>" name="printfiltercheck" />
    <input type="hidden" value="<?php echo @$_POST['peo']?>" name="peo" />
...
<button type="submit" class="btn btn-danger" name="deletecheck"><span class="glyphicon glyphicon-trash"></span> Delete Entries</button></span></div>
 <div id="txtHint">
  <p >PEO : <strong style="color:red;"><?php echo @$_POST['peo']?></strong></p>
 ...
 if(isset($_POST['filtercheck'])){
	 $que = @mysql_real_escape_string($_POST['peo']);
	 $stat = mysql_real_escape_string(trim($_POST['filtercheck']));
 	$sql  = "SELECT * FROM cf_inventory WHERE peo = '$que' AND status = '$stat'";
	$res = mysql_query($sql) or die (mysql_error());
	$count = 0;
	$stat = '';
	$statclass = '';
 ...
 if(mysql_num_rows($res)>0){
	...
	if($row['status']!=''){
		$statclass = 'warning';
			...
			$statclass = 'success';
			}else if($row['status']=='Un-claimed'){$statclass = 'danger';}else {$statclass = '';}
 ?>
        ...
        <th style="width:auto;"><?php echo $row['checkdate'];?></th>
    ...
    <?php 
	 /*?>include('cf_delete_modal.php');<?php */?>
        ...
        <?php }} else {echo '<strong style="color:red;">No '.$stat.' check available on '.$que.' PEO</strong>';}?>
        </tbody>
