...
<div class="panel panel-default" style="font-family:'Comic Sans MS', cursive;">
  ...
  <div class="panel-body">
    ...
    function totalAmount(){
    	...
    	<form class="form-horizontal" method="post" action="ff_editprocess">
        <?php include('includes/dbconn.php');
		$id = intval($_GET['id']);
		$query = mysql_query("SELECT * FROM fixtureandfurniture WHERE id = '$id'") or die (mysql_error());
		if(mysql_num_rows($query)>0){
			while($row=mysql_fetch_array($query)){ ?>
 <div class="col-lg-2">
 <a href="ffTable" class="btn btn-danger btn-block">Back</a></div>
 <div class="col-lg-8">
  ...
  <input type="hidden" name="id" value="<?php echo $row['id'];?>" />
   <input type="hidden" name="idno" value="<?php echo $row['ffid'];?>" />
