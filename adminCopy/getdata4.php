...
$dis = '';
$q = mysql_query("SELECT * FROM transportation_equipment WHERE tid = '$que'") or die (mysql_error());
if(mysql_num_rows($q)>0){
	while($r = mysql_fetch_array($q)){
			if($r['m_dep']==0 && $r['e_life']==0){
				$dis = 'Disabled';}else {$dis = 'Enabled';}
		}}
$sql=mysql_query("SELECT * FROM transportation_ledgercard WHERE tid LIKE '$que'") or die (mysql_error());
...
?>
 ...
 <table id="example" class="table table-responsive table-hover"  width="100%">
		 ...
		 if(mysql_num_rows($sql)==0){?>  
           ...
           <td style="text-align:center; width:auto;background-color:#CFC;"><?php echo $row['cost'];?></td>
   
           <td style="text-align:center; width:auto; background-color:#CFC; "><?php echo $row['bookvalue'];?></td> 
           <td style="text-align:center; width:auto; background-color:#CCC;"><a  href="myModalupdateledgertrans?id=<?php echo $row['id']; ?>" class="btn btn-warning btn-sm" data-toggle="tooltip" title="Update?"><span style="color:#FFF;"><i class="glyphicon glyphicon-edit"></i> Update</span></a> | <a  href="deletetransledger_process?id=<?php echo $row['id']; ?>" <?php echo $dis;?> class="btn btn-danger btn-sm" data-toggle="tooltip" title="Delete?"><span style="color:#FFF;"><i class="glyphicon glyphicon-trash"></i> Delete</span></a> </td>
