 ...
 <p style="font-size:12px;">
          ...
          <thead style="border:solid 1px; font-size:12px;">
            ...
            <tr style="color:#F30; border:solid 1px;">
                <td colspan="9">&nbsp;<strong><u>FURNITURE AND FIXTURE EQUIPMENT</u></strong></td></tr>
			...
			$bval = 0;
					...
					$a = 0;
					$b=0;
			 $q = mysql_query("SELECT ff_ledgercard.itemname AS itemname,ff_ledgercard.ffid AS ffid ,ff_ledgercard.cost AS cost,
								 ff_ledgercard.depreciation AS depreciation,
								 ff_ledgercard.accum_deplm AS paccumdep, ff_ledgercard.accum_deptm AS accumdep,ff_ledgercard.date,
								 ff_ledgercard.bookvalue as bookval,fixtureandfurniture.ffid,fixtureandfurniture.qty,fixtureandfurniture.s_val,fixtureandfurniture.m_dep,fixtureandfurniture.e_life
								  FROM ff_ledgercard LEFT JOIN fixtureandfurniture ON 
								  ff_ledgercard.ffid = fixtureandfurniture.ffid WHERE fixtureandfurniture.m_dep =0 AND fixtureandfurniture.e_life =0") or die ( mysql_error());
            ...
            <tr style="font-size:12px;border:solid 1px;">
                ...
                <td style="text-align:center;border:solid 1px;"><?php echo number_format($r['qty'],2,'.',',');?></td>
            ...
            <?php include('includes/dbconn.php');
					...
					$sql = "SELECT ff_ledgercard.itemname AS itemname,ff_ledgercard.ffid AS ffid ,ff_ledgercard.cost AS cost,
								 ff_ledgercard.depreciation AS depreciation,
								 ff_ledgercard.accum_deplm AS paccumdep, ff_ledgercard.accum_deptm AS accumdep,ff_ledgercard.date,
								 ff_ledgercard.bookvalue as bookval,fixtureandfurniture.ffid,fixtureandfurniture.qty,fixtureandfurniture.s_val,fixtureandfurniture.m_dep,fixtureandfurniture.date_acquired
								  FROM ff_ledgercard LEFT JOIN fixtureandfurniture ON 
								  ff_ledgercard.ffid = fixtureandfurniture.ffid WHERE year(ff_ledgercard.date) LIKE '$year%' AND  Month(ff_ledgercard.date) = '$m'";
...
header("Content-Disposition: attachment; filename=.$month.$year. FURNITURE AND FIXTURE PPE Report.xls");
