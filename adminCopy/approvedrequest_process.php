...
$id = intval($_GET['itemno']);
		...
		while( $row = mysql_fetch_assoc($q)){
			...
			$s = mysql_query("INSERT INTO pcic_stockinventory VALUES(NULL,'$itemname','$itemqty','$unitcost','$totalcost','$qty','$unitcost','$tcost','$itembalqty','$unitcost','$baltotalcost','$timestamp','$itemno')") or die(mysql_error());
		}
	
	if ($query){
	
	$ql = mysql_query("UPDATE pcic_items SET bal_qty = '$newqty',
					                         bal_totalcost = '$newtcost' WHERE itemno= '$itemno' ") or die(mysql_error());

    $sl = mysql_query("INSERT INTO  pcic_stockcard VALUES(NULL,'$itemno','$itemname','$des','$unit','$dater','$itemqty','$ucost','$totalcost1','$dep','$qty','$ucost','$totalissuedcost',NULL,'$newqty','$ucost','$newtcost','$timestamp')") or die (mysql_error());
	  
   $_SESSION['success_msg'] = '<strong><span class="glyphicon glyphicon-ok"></span>'.$itemname.' Request updated!</strong>';
