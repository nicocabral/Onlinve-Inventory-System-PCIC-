...
<div class="modal fade" id="modal" tabindex="-1" role="dialog"  aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="font-family:'Comic Sans MS', cursive;">
       ...
       <div class="row">
        <div class="col-md-9">
        <span style="font-size:23px; color: #fff; font-weight:bold;"><img src="/PCIC/img/icon/update.png" width="50px"/> EDIT EDP LEDGERCARD</span>
       </div> 
<div class="col-md-2">
...
<?php unset($_SESSION['success_msg']);
	  unset($_SESSION['class']); } ?>
		...
		$id = @intval($_GET['id']);
		$sql = @mysql_query("SELECT * FROM edp_ledgercard WHERE id = '$id'") or die (mysql_error());
    ...
    <label for="inputEmail3" class="col-sm-2 control-label">Date</label>
      ...
      <input type="hidden" class="form-control" name="idno" value="<?php echo $row['eno'];?>">
    </div>
  </div>
   <div class="col-lg-6">
  <div class="form-group">
  ...
  <div class="row">
   ...
   <div class="form-group">
    ...
    <label for="inputPassword3" class="col-sm-2 control-label" data-toggle="tooltip" title="Accumulated Depreciation Last Month">ADLM</label>
      ...
      <input type="number" class="form-control"  placeholder="<?php echo $row['accumdep_lastmonth'];?>" readonly="readonly" data-toggle="tooltip" title="Accumulated Depreciation Last Month">
    </div>
  </div>
  </div>
  <div class="row">
   ...
   <div class="form-group">
    ...
    <label for="inputPassword3" class="col-sm-2 control-label" data-toggle="tooltip" title="Accumulated Depreciation This Month">ADTM</label>
      ...
      <input type="number" class="form-control"  placeholder="<?php echo $row['accumdep_thismonth'];?>" readonly="readonly" data-toggle="tooltip" title="Accumulated Depreciation This Month">
    </div>
  </div>
   <div class="col-lg-6">
   <div class="form-group">
  ...
  <?php  }?>
      ...
      <div class="modal-footer">
        <a href="edp" type="button" class="btn btn-default"><span class="glyphicon glyphicon-remove-sign"></span> Close</a>
		...
		if(isset($_POST['save'])){
			...
			$eno = 0;
			$checkdate = mysql_query("SELECT * FROM edp_ledgercard WHERE id = '$id'") or die (mysql_error());
				if($rows = mysql_fetch_array($checkdate)){
					$d= $rows['date'];
					$eno = $row['eno'];
						$sql = mysql_query("SELECT date,eno FROM edp_ledgercard WHERE eno = '$eno' GROUP by eno") or die (mysql_error());
    ...
    header("Location:myModalupdateledger?id=$id");
	...
	exit();										
			...
			$query = mysql_query("SELECT date,eno FROM edp_ledgercard WHERE Year(date) = '$dd' AND Month(date) > '$e' AND eno = '$eno'") or die (mysql_error());
	...
	$_SESSION['class'] = 'danger';exit();
								...
								$queryupdate = mysql_query("UPDATE edp_ledgercard SET date = '$date' WHERE id = '$id'") or die (mysql_error());
					if($queryupdate){	
...
//				if($rows=mysql_fetch_array($chekdate)){
//						$dated = new DateTime($rows['date']);
//						$eno = $eno + $rows['eno'];
//					}
//				$s = mysql_query("SELECT eno FROM edp_ledgercard WHERE eno = '$eno'") or die (mysql_error());
//				if(mysql_num_rows($s)>0){
//					if($r = mysql_fetch_array($s)){
//							$eno = $r['eno'];
//						$a = mysql_query("SELECT date,eno FROM edp_ledgercard WHERE Year(date)>'$dd'") or die (mysql_error());
//							if(mysql_num_rows($a)>0){
//								}
//						
//						}}
//				
//				$_SESSION['success_msg'] = '<img src="/PCIC/adminCopy/delete.png" style="width:20px;"><strong> Oooops! Sory, &nbsp;'.$name.' Unable to update on &nbsp;' .$isdate->format('M d, Y').$eno.'</strong>';
//    header("Location:myModalupdateLedger?id=$id");
//	$_SESSION['class'] = 'danger';exit();
//				}
//				$query = mysql_query("UPDATE edp_ledgercard SET date = '$date' WHERE id = '$id'") or die (mysql_error());
//				if($query){
//					 $_SESSION['success_msg'] = '<img src="/PCIC/img/icons/ok2.ico" style="width:20px;"><strong> Updated on ' .$isdate->format('M d, Y'). ' Updated successfully</strong>';				
//					header("location:myModalupdateLedger?id=$id");
//					$_SESSION['class'] = 'success';}else {
//						$_SESSION['success_msg'] = '<strong><span class="glyphicon glyphicon-remove-sign></span></strong> Sorry Unable to process your request"';
//					
//					header("location:edp");
//					$_SESSION['class'] = 'danger';
