...
<?php unset($_SESSION['success_msg']);
       ...
       <a href="risTable" data-toggle="tooltip" title="AFD RIS" class="btn btn-success" data-placement="bottom">AFD &emsp;<span class="badge">
					...
					include('includes/dbconn.php'); $query = mysql_query("SELECT COUNT(*) as newreqafd FROM pcic_requestitem WHERE status='Pending' AND department ='AFD'") or die (mysql_error());
						if($rows = mysql_fetch_array($query)){
                            ...
                            <a href="risTablecad" data-toggle="tooltip" title="CAD RIS" class="btn btn-primary" data-placement="bottom">CAD &emsp;<span class="badge">
        			<?php
					include('includes/dbconn.php'); $query = mysql_query("SELECT COUNT(*) as newreqafd FROM pcic_requestitem WHERE status='Pending' AND department ='CAD'") or die (mysql_error());
						if($rows = mysql_fetch_array($query)){
 ...
 <?php include('includes/dbconn.php');
 	$sql = mysql_query("SELECT * FROM pcic_requestitem WHERE status !=' ' AND department='MSD' ORDER by timestamp desc") or die (mysql_error());
	$count = 0;
	while($row = mysql_fetch_assoc($sql)){
 	$count++;
 ?>
        <tr class="table-row warning" data-href="risTablemsd">
