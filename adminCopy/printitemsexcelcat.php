...
<center>
 ...
 $otbal = 0;
	...
	$cat = mysql_real_escape_string(trim($_POST['category']));
 	$sql = "SELECT pcic_items.*,pcic_stockcard.stockno,pcic_stockcard.date_recieved FROM pcic_items LEFT JOIN pcic_stockcard ON pcic_items.itemno = pcic_stockcard.stockno WHERE Year(pcic_stockcard.date_recieved) = '$year' AND pcic_items.category = '$cat' group by pcic_items.itemno";
...
header("Content-Disposition: attachment; filename=office supplies inventory - $year $cat.xls");
