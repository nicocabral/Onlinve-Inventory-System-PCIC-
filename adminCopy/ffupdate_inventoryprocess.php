...
include('includes/dbconn.php');
	if(isset($_POST['save'])){
		...
		$check = mysql_query("SELECT date FROM ff_ledgercard WHERE  ffid = '$eno' Group by ffid") or die (mysql_error());
	...
	$_SESSION['class'] = 'danger';exit();}else {
		$sql = mysql_query("SELECT ffid,cost,date,depreciation,accum_deplm,accum_deptm,bookvalue FROM ff_ledgercard WHERE ffid = '$eno'") or die (mysql_error());
		...
		$checkdate = mysql_query("SELECT date,ffid FROM ff_ledgercard WHERE Year(date) = '$dd' AND Month(date) = '$e' AND ffid = '$eno'") or die (mysql_error());
	...
	$_SESSION['class'] = 'danger';exit();
		...
		if(number_format($bval,0,'.','')<number_format($sval,0,'.','')){
			$depreciated = "INSERT INTO ff_depreciated VALUES(NULL,'$eno','$name','$datedep','$qty','$amount',0,'$deptm','$deptm','$bookvalue','Depreciated','$dateacquired')";
	...
	$_SESSION['class'] = 'danger';exit();}
				...
				$q = "INSERT INTO ff_ledgercard VALUES(NULL,'$eno','$name','$date','$amount','$dep','$b','$a','$bval')";
...
//				}
	...
	$_SESSION['class'] = 'danger';exit();
				...
				$count = mysql_query("SELECT COUNT(*) as total FROM ff_ledgercard WHERE ffid = '$eno'") or die (mysql_error());
