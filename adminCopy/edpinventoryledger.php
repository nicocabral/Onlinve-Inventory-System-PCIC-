...
<div class="col-lg-3">
	<form method="post" action="edpinventoryledgerexcel">
...
<div class="col-lg-1">
<a href="edpTable" data-toggle="tooltip" title="Back to previous page" class="btn btn-danger" style="padding-left: 10px;" data-placement="bottom">Back</a></div>
 </div>
...
<div style="text-align:center;">
            ...
            <span class="pull-right">NO.<?php
				include('includes/dbconn.php');
				$itemname = intval(trim($_GET['id']));
				 $s = mysql_query("SELECT eno FROM edp_ledgercard WHERE eno = '$itemname'") or die (mysql_error());
				if($r=mysql_fetch_array($s)){ echo '<strong>'.$r['eno'].'</strong>';}?></span>
           
            <table width="100%" style="border:1px solid;">
   
            <tr style="font-size:12px;border:1px solid;">
            <td  colspan="8" style="font-size:12px;"><p style="font-size:12px;">&nbsp;Property, Plant and Equipment: &emsp;<?php include('includes/dbconn.php');
								$query  = mysql_query("SELECT * FROM edp_equipement where e_no = '$itemname'") or die (mysql_error());
								if($rows = mysql_fetch_array($query)){echo '<strong>'.$rows['itemname'].'</strong>';?><p>
            													 </td>
            <th style="width:auto;border:1px solid;" colspan="2"><p style="font-size:8px; text-align:left;">Account Code:<br />
            																			 Property No.:<br />
                                                										 Est. Useful Life: <strong style="color:red;"><?php echo $rows['e_life'].' Months';?></strong><br />
                                                										 Rate of Depreciation:</p></th>
                                                                                         <?php } ?>
       <tr style="font-size:12px;border:1px solid;"><td colspan="8" style="font-size:12px; border:1px solid;">&nbsp;Description :  &emsp;<?php include('includes/dbconn.php');
								$query  = mysql_query("SELECT * FROM edp_equipement where e_no = '$itemname'") or die (mysql_error());
								if($rows = mysql_fetch_array($query)){echo '<strong>'.$rows['description'].'</strong>';}?></td>
       		<td colspan="2" style="text-align:center;border:1px solid;">Maintenance Expenses</td></tr>
            </tr>
             <tr style="font-size:12px;border:1px solid;">
             <th style="text-align:center;border:1px solid; width:100px;">DATE</th>
            ...
            <?php include('includes/dbconn.php');
				$sql = "SELECT * FROM edp_ledgercard WHERE eno ='$itemname' order by date";
				...
				if(mysql_num_rows($res)>0){
					while($row=mysql_fetch_array($res)){
						...
						$accumdep = $row['accumdep_thismonth'];
						$val = $row['bookvalue'];
						?>
            <tr style=" font-size:12px;border:1px solid;">
            ...
            <td style="text-align:center;border:1px solid;"><?php echo number_format($row['accumdep_thismonth'],2,'.',',');?></td>
            ...
            <td style="background-color:#039; color:#FFF; text-align:right;border:1px solid #000;" colspan="3">TOTAL</td>
            ...
            <td style="color:#F30; text-align:center;border:1px solid;"></td>
            <td style="color:#F30; text-align:center;border:1px solid;"><strong><?php echo number_format($val,2,'.',',');?></strong></td>
            <td style="color:#F30; text-align:center;border:1px solid;" colspan="2"></td></tr>
