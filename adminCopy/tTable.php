...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content" style=" font-family:'Comic Sans MS', cursive;">
      ...
      <div class="row">
        <div class="col-md-9">
        <span style="font-size:23px; color: #fff; font-weight:bold;"><img src="/PCIC/img/icon/printer.ico" width="45px"/>TRANSPORTATION EQUIPMENT ITEMS</span>
       </div> 
<div class="col-md-2">
    <div class="a_close"><a  href="#"  data-toggle="tooltip" title="Close" class="pull-right" data-dismiss="modal"><span class="glyphicon glyphicon-remove"></span></a></div><br>
      ...
      <div class="row">
      <div class="col-lg-12">
       <form class="form-inline" method="post" action="trans_printitems">
...
?>
 ...
 <?php include('includes/dbconn.php');
 	$sql = mysql_query("SELECT * FROM transportation_equipment") or die (mysql_error());
	$count = 0;
	$dis = '';
	...
	$dep = mysql_query("SELECT * FROM t_depreciated WHERE eno = '$tid'") or die (mysql_error());
	if($deprow = mysql_fetch_array($dep)){
	if($deprow['status'] == 'Depreciated'){
		$dis = 'disabled';
		}}
 ?>		
        <tr class="table-row" data-href="tinventoryledger?id=<?php echo $row['tid'];?>" data-toggle="tooltip" title="<?php $query = "SELECT date FROM transportation_ledgercard WHERE tid ='$tid' order by date desc";
       ...
       <td style="background-color: #F30; color: #FFF;text-align:center; width:auto;"><strong><?php echo number_format($row['amount'],2,'.',',');?></strong></td>
       ...
       <td style="background-color:#CFC; color: #000; text-align:center; font-weight:bold;"><?php echo number_format($row['m_dep'],2,'.',',');?></td>
        <td style="text-align:center; width:auto; background-color:#CCC;"><a href="edittrans?id=<?php echo $row['id']?>"  class="btn btn-warning"><span class="glyphicon glyphicon-edit" data-toggle="tooltip" title="Edit?"></span> Edit</a> | <a href="#deleteModal<?php echo $row['id'];?>" class="btn btn-danger" data-toggle="modal" data-target="#deleteModal<?php echo $row['id'];?>"><span class="glyphicon glyphicon-trash" data-toggle="tooltip" title="Delete?"></span> Del</a> </td>
        </tr>
   		<?php 
			  include('tdelete.php');
			  include('updatet.php');
