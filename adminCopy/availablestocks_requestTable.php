...
<?php unset($_SESSION['success_msg']);
 ...
 ?>
        ...
        <th style="background-color: #F30; color: #FFF; text-align:center; font-weight:bold;"><?php echo $row['qty'];
		$bal = $row['qty'];
		if($bal<=0){
		$row['unit_cost'] =0;
		$row['bal_totalcost']=0;}
        ...
        <th style="background-color: #F30; color: #FFF; text-align:center; width:auto;"><?php echo $row['bal_qty'];?></th>
		...
		$da = new DateTime($row['date_arrival']);
		echo $da->format('M d, Y');?></th>
        <th style="text-align:center; width:auto; background-color:#CFC"><?php
		$stat = $row['bal_qty'];
		$status = '';
		if ($stat<=0){
		$status='out of stock';
