...
<div class="modal fade" id="modal" tabindex="-1" role="dialog"  aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="font-family:'Comic Sans MS', cursive;">
       ...
       <div class="row">
        <div class="col-md-9">
        <span style="font-size:23px; color: #fff; font-weight:bold;"><img src="/PCIC/img/icon/update.png" width="50px"/> EDIT OFFICE LEDGERCARD</span>
       </div> 
<div class="col-md-2">
...
<?php unset($_SESSION['success_msg']);
	  unset($_SESSION['class']); } ?>
		...
		$id = @intval($_GET['id']);
		$sql = @mysql_query("SELECT * FROM office_ledgercard WHERE id = '$id'") or die (mysql_error());
  ...
  <?php  }?>
      ...
      <div class="modal-footer">
        <a href="office_equipment_page" type="button" class="btn btn-default"><span class="glyphicon glyphicon-remove-sign"></span> Close</a>
		...
		if(isset($_POST['save'])){
			...
			$eno = 0;
			$checkdate = mysql_query("SELECT * FROM office_ledgercard WHERE id = '$id'") or die (mysql_error());
				if($rows = mysql_fetch_array($checkdate)){
					$d= $rows['date'];
					$eno = $row['oid'];
						$sql = mysql_query("SELECT date,oid FROM office_ledgercard WHERE oid = '$eno' GROUP by oid") or die (mysql_error());
    ...
    header("Location:myModalupdateledgeroffice?id=$id");
	...
	exit();										
			...
			$query = mysql_query("SELECT date,oid FROM office_ledgercard WHERE Year(date) = '$dd' AND Month(date) > '$e' AND oid = '$eno'") or die (mysql_error());
	...
	$_SESSION['class'] = 'danger';exit();
								...
								$queryupdate = mysql_query("UPDATE office_ledgercard SET date = '$date' WHERE id = '$id'") or die (mysql_error());
					if($queryupdate){	
